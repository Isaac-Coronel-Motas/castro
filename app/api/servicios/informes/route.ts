    ...
    const resumenQuery = `
      SELECT 
        ...
        COALESCE(AVG(
          ...
          END
        ), 0.0) as tiempo_promedio_resolucion
...
// POST /api/servicios/informes - Obtener informe de servicios técnicos con filtros en body
export async function POST(request: NextRequest) {
  try {
    ...
    const filtros: FiltrosInformeServicios = await request.json();
    
    const fecha_desde = filtros.fecha_inicio;
    const fecha_hasta = filtros.fecha_fin;
    const sucursal_id = filtros.sucursal_id;
    const tecnico_id = filtros.tecnico_id;
    const cliente_id = filtros.cliente_id;
    const estado = filtros.estado;
    const tipo_periodo = filtros.tipo_periodo || 'mes';
    ...
    const sucursalFilter = sucursal_id && sucursal_id > 0
      ? `AND sucursal_id = ${sucursal_id}`
      : '';

    const tecnicoFilter = tecnico_id && tecnico_id > 0
      ? `AND tecnico_id = ${tecnico_id}`
      : '';

    const clienteFilter = cliente_id && cliente_id > 0
      ? `AND cliente_id = ${cliente_id}`
      : '';
