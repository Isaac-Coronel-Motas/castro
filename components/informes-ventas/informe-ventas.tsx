...
export default function InformeVentasComponent() {
  ...
  const handleFilterChange = (key: keyof FiltrosInformeVentas, value: any) => {
    setFiltros(prev => ({
      ...prev,
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Filtros de Informe de Ventas
          </CardTitle>
          <CardDescription>
            Configure los filtros para personalizar el informe de ventas
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={filtros.tipo_documento || 'all'} onValueChange={(value) => handleFilterChange('tipo_documento', value)}>
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select value={filtros.condicion_pago || 'all'} onValueChange={(value) => handleFilterChange('condicion_pago', value)}>
                <SelectTrigger>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{formatNumber(informe.resumen.total_transacciones)}</div>
                <div className="flex items-center text-xs text-muted-foreground">
                  {getTrendIcon(informe.resumen.tendencia_transacciones)}
                  <span className={`ml-1 ${getTrendColor(informe.resumen.tendencia_transacciones)}`}>
                    {Math.abs(informe.resumen.tendencia_transacciones)}% vs período anterior
                  </span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{formatCurrency(informe.resumen.promedio_venta)}</div>
                <div className="flex items-center text-xs text-muted-foreground">
                  {getTrendIcon(informe.resumen.tendencia_promedio)}
                  <span className={`ml-1 ${getTrendColor(informe.resumen.tendencia_promedio)}`}>
                    {Math.abs(informe.resumen.tendencia_promedio)}% vs período anterior
                  </span>
                </div>
              </CardContent>
            </Card>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {informe.distribucion_por_estado.map((item, index) => (
                  <div key={index} className="text-center">
                    <div className="text-2xl font-bold">{formatNumber(item.cantidad)}</div>
                ...
                {informe.top_clientes.map((cliente, index) => (
                  <div key={index} className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <div className="font-medium">{formatCurrency(cliente.total_ventas)}</div>
                      <div className="text-sm text-muted-foreground">{cliente.total_transacciones} transacciones</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Ventas distribuidas por tipo de documento</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {informe.distribucion_por_tipo_documento.map((tipo, index) => (
                  <div key={index} className="text-center p-4 border rounded-lg">
                    <div className="text-lg font-semibold">{tipo.tipo_documento}</div>
                    <div className="text-2xl font-bold text-primary">{formatCurrency(tipo.total_ventas)}</div>
                    <div className="text-sm text-muted-foreground">{tipo.total_transacciones} transacciones</div>
                  </div>
                ))}
              </div>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Distribución por Condición de Pago</CardTitle>
              <CardDescription>Ventas distribuidas por condición de pago</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {informe.distribucion_por_condicion_pago.map((condicion, index) => (
                  <div key={index} className="text-center p-4 border rounded-lg">
                    <div className="text-lg font-semibold">{condicion.condicion_pago}</div>
                    <div className="text-2xl font-bold text-primary">{formatCurrency(condicion.total_ventas)}</div>
                    <div className="text-sm text-muted-foreground">{condicion.total_transacciones} transacciones</div>
                  </div>
                ))}
              </div>
            </CardContent>
              ...
              <div className="space-y-4">
                {informe.tendencias_mensuales.map((tendencia, index) => (
                  <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                    ...
                    <div className="text-right">
                      <div className="font-medium">{formatCurrency(tendencia.total_ventas)}</div>
