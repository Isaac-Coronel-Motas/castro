...
export function InformeRegistroComponent() {
  const authenticatedFetch = useAuthenticatedFetch()
  const [informe, setInforme] = useState<InformeRegistroCompras | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [filtros, setFiltros] = useState<FiltrosInformeRegistro>({
    fecha_desde: '',
    fecha_hasta: '',
  ...
  const loadInitialData = async () => {
    try {
      const [sucursalesRes, proveedoresRes, tiposDocRes] = await Promise.all([
        ...
        authenticatedFetch.authenticatedFetch('/api/tipos-documento')
      ])
  ...
  const loadInforme = async () => {
    ...
    try {
      ...
      if (filtros.tipo_doc_id) params.append('tipo_doc_id', filtros.tipo_doc_id.toString())
      if (filtros.nro_factura) params.append('nro_factura', filtros.nro_factura)
      ...
      const response = await authenticatedFetch.authenticatedFetch(`/api/compras/informes/registro?${params.toString()}`)
  ...
  const handleFiltroChange = (field: keyof FiltrosInformeRegistro, value: string | number | undefined) => {
    setFiltros(prev => ({
      ...prev,
      [field]: value
    }))
  ...
  const handleExportarInforme = () => {
    console.log('Exportar informe de registro:', informe)
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Filtros del Informe de Registro de Compras
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={filtros.tipo_doc_id?.toString() || 'all'}
                onValueChange={(value) => handleFiltroChange('tipo_doc_id', value === 'all' ? undefined : parseInt(value))}
              >
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => handleFiltroChange('nro_factura', e.target.value)}
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select
                value={filtros.condicion_pago || 'all'}
                onValueChange={(value) => handleFiltroChange('condicion_pago', value === 'all' ? '' : value)}
              >
                <SelectTrigger>
      ...
      {informe && !loading && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <Card className="hover:shadow-lg transition-all duration-300">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-muted-foreground mb-1">Promedio por Compra</p>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {formatNumber(proveedor.cantidad_registros)} compras ({proveedor.porcentaje.toFixed(1)}%)
                          </p>
                        </div>
                      </div>
          ...
          {informe.por_tipo_documento && informe.por_tipo_documento.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
              ...
              <CardContent>
                <div className="space-y-4">
                  {informe.por_tipo_documento.map((tipo, index) => (
                    <div key={index} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <Badge variant="outline">{tipo.tipo_doc_nombre}</Badge>
                        <span className="text-sm text-muted-foreground">
                          {formatNumber(tipo.cantidad)} ({tipo.porcentaje.toFixed(1)}%)
                        </span>
                      </div>
                      <div className="text-right">
                        <p className="font-medium">{formatCurrency(tipo.valor_total)}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {informe.por_condicion_pago && informe.por_condicion_pago.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
              ...
              <CardContent>
                <div className="space-y-4">
                  {informe.por_condicion_pago.map((condicion, index) => (
                    <div key={index} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <Badge variant="outline">{condicion.condicion_pago}</Badge>
                        <span className="text-sm text-muted-foreground">
                          {formatNumber(condicion.cantidad)} ({condicion.porcentaje.toFixed(1)}%)
                        </span>
                      </div>
                      <div className="text-right">
                        <p className="font-medium">{formatCurrency(condicion.valor_total)}</p>
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    <div key={index} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {formatNumber(sucursal.cantidad_registros)} compras ({sucursal.porcentaje.toFixed(1)}%)
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
