...
export function InformeOrdenesComponent() {
  ...
  const loadInforme = async () => {
    ...
    try {
      ...
      const response = await authenticatedFetch.authenticatedFetch(`/api/compras/informes/ordenes?${params.toString()}`)
  ...
  const handleFiltroChange = (field: keyof FiltrosInformeOrdenes, value: string | number | undefined) => {
    setFiltros(prev => ({
      ...prev,
      [field]: value
    }))
  ...
  const handleExportarInforme = () => {
    console.log('Exportar informe de órdenes:', informe)
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Filtros del Informe de Órdenes de Compra
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={filtros.tipo_documento || 'all'}
                onValueChange={(value) => handleFiltroChange('tipo_documento', value === 'all' ? '' : value)}
              >
                <SelectTrigger>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-foreground">{formatCurrency(informe.resumen.monto_oc)}</p>
                  </div>
            ...
            <Card className="hover:shadow-lg transition-all duration-300">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-muted-foreground mb-1">Promedio por Orden</p>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {formatNumber(proveedor.cantidad_registros)} órdenes ({proveedor.porcentaje.toFixed(1)}%)
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-medium">{formatCurrency(proveedor.monto_oc)}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-4">
                  {informe.por_condicion_pago.map((condicion, index) => (
                    <div key={index} className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <p className="font-medium">{formatCurrency(condicion.monto_oc)}</p>
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    <div key={index} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {formatNumber(sucursal.cantidad_registros)} órdenes ({sucursal.porcentaje.toFixed(1)}%)
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-medium">{formatCurrency(sucursal.monto_oc)}</p>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {informe.tendencias_mensuales.slice(0, 12).map((mes, index) => (
                    <div key={index} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-muted-foreground">{formatCurrency(mes.monto_oc)}</span>
