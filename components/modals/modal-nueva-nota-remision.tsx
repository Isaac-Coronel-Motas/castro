...
interface ModalNuevaNotaRemisionProps {
  onNotaRemisionCreated: () => void;
}

export function ModalNuevaNotaRemision({ onNotaRemisionCreated }: ModalNuevaNotaRemisionProps) {
  ...
  const [errors, setErrors] = useState<Partial<NotaRemisionCreate>>({});
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<NotaRemisionCreate> = {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const notaRemisionData: NotaRemisionCreate = {
        ...formData,
        fecha_remision: formData.fecha_remision || new Date().toISOString().split('T')[0],
      ...
      const result = await createNotaRemision(notaRemisionData);
      
      if (result) {
        toast({
          title: 'Nota de remisión creada',
          description: `Nota de remisión ${result.codigo_remision} creada exitosamente`,
        });
        
        setOpen(false);
        resetForm();
        onNotaRemisionCreated();
      } else {
        toast({
          title: 'Error',
          description: createError || 'Error al crear la nota de remisión',
          variant: 'destructive',
        });
      }
    } catch (error) {
      console.error('Error creating nota remision:', error);
      toast({
        title: 'Error',
        description: 'Error inesperado al crear la nota de remisión',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
