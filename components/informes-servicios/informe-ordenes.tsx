...
import { InformeOrdenes, FiltrosInformeOrdenes } from '@/lib/types/informes-servicios'

export function InformeOrdenesComponent() {
  const authenticatedFetch = useAuthenticatedFetch()
  const [informe, setInforme] = useState<InformeOrdenes | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [filtros, setFiltros] = useState<FiltrosInformeOrdenes>({
    fecha_desde: '',
    fecha_hasta: '',
    ...
    estado: '',
    forma_cobro_id: undefined,
    fecha_ejecucion_desde: '',
    fecha_ejecucion_hasta: '',
    tipo_periodo: 'mes'
  })
  ...
  const loadInitialData = async () => {
    try {
      const [sucursalesRes, tecnicosRes, clientesRes, formasRes] = await Promise.all([
        ...
        authenticatedFetch.authenticatedFetch('/api/servicios/referencias/formas-cobro')
      ])
      ...
      if (formasData.success) {
        setFormasCobro(formasData.data)
      }
    } catch (error) {
  ...
  const loadInforme = async () => {
    ...
    try {
      ...
      if (filtros.forma_cobro_id) params.append('forma_cobro_id', filtros.forma_cobro_id.toString())
      if (filtros.fecha_ejecucion_desde) params.append('fecha_ejecucion_desde', filtros.fecha_ejecucion_desde)
      if (filtros.fecha_ejecucion_hasta) params.append('fecha_ejecucion_hasta', filtros.fecha_ejecucion_hasta)
      if (filtros.tipo_periodo) params.append('tipo_periodo', filtros.tipo_periodo)

      const response = await authenticatedFetch.authenticatedFetch(`/api/servicios/informes/ordenes?${params.toString()}`)
  ...
  const handleFiltroChange = (key: keyof FiltrosInformeOrdenes, value: any) => {
    setFiltros(prev => ({
      ...prev,
      [key]: value
    }))
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Filtros del Informe de Órdenes</CardTitle>
          <CardDescription>
            Configure los filtros para generar el informe de órdenes de servicio
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={filtros.forma_cobro_id?.toString() || 'all'}
                onValueChange={(value) => handleFiltroChange('forma_cobro_id', value === 'all' ? undefined : parseInt(value))}
              >
                <SelectTrigger>
      ...
      {informe && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Técnicos con más órdenes</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Clientes con más órdenes</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
          ...
          {/* Distribución por Sucursal y Forma de Cobro */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                <CardTitle>Distribución por Sucursal</CardTitle>
                <CardDescription>Órdenes por sucursal</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
            ...
            <Card>
              <CardHeader>
                <CardTitle>Distribución por Forma de Cobro</CardTitle>
                <CardDescription>Órdenes por forma de cobro</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {informe.por_forma_cobro.map((forma, index) => (
                    <div key={index} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="font-medium">{forma.estado}</span>
                      </div>
                      <div className="text-right">
                        <div className="font-medium">{formatNumber(forma.cantidad)}</div>
                        <div className="text-sm text-gray-500">{forma.porcentaje}%</div>
                        {forma.valor_total && (
                          <div className="text-sm text-gray-500">{formatCurrency(forma.valor_total)}</div>
                        )}
                      </div>
                    </div>
                  ))}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Evolución de las órdenes en los últimos meses</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
