...
import { InformeSolicitudes, FiltrosInformeSolicitudes } from '@/lib/types/informes-servicios'

export function InformeSolicitudesComponent() {
  const authenticatedFetch = useAuthenticatedFetch()
  const [informe, setInforme] = useState<InformeSolicitudes | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [filtros, setFiltros] = useState<FiltrosInformeSolicitudes>({
    fecha_desde: '',
    fecha_hasta: '',
  ...
  const loadInitialData = async () => {
    try {
      const [sucursalesRes, tecnicosRes, clientesRes, ciudadesRes] = await Promise.all([
        ...
        authenticatedFetch.authenticatedFetch('/api/referencias/ciudades')
      ])
  ...
  const loadInforme = async () => {
    ...
    try {
      ...
      if (filtros.tipo_atencion) params.append('tipo_atencion', filtros.tipo_atencion)
      if (filtros.ciudad_id) params.append('ciudad_id', filtros.ciudad_id.toString())
      if (filtros.tipo_periodo) params.append('tipo_periodo', filtros.tipo_periodo)

      const response = await authenticatedFetch.authenticatedFetch(`/api/servicios/informes/solicitudes?${params.toString()}`)
  ...
  const handleFiltroChange = (key: keyof FiltrosInformeSolicitudes, value: any) => {
    setFiltros(prev => ({
      ...prev,
      [key]: value
    }))
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Filtros del Informe de Solicitudes</CardTitle>
          <CardDescription>
            Configure los filtros para generar el informe de solicitudes de servicio
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={filtros.tipo_atencion || 'all'}
                onValueChange={(value) => handleFiltroChange('tipo_atencion', value === 'all' ? '' : value)}
              >
                <SelectTrigger>
            ...
            <div className="space-y-2">
              <Label>Ciudad</Label>
              <Select
                value={filtros.ciudad_id?.toString() || 'all'}
                onValueChange={(value) => handleFiltroChange('ciudad_id', value === 'all' ? undefined : parseInt(value))}
              >
                <SelectTrigger>
      ...
      {informe && (
        <>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Distribución por Tipo de Atención</CardTitle>
              <CardDescription>Tipo de atención solicitada</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {informe.por_tipo_atencion.map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Técnicos con más solicitudes</CardDescription>
              </CardHeader>
              <CardContent>
          ...
          {/* Distribución por Sucursal y Ciudad */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                <CardTitle>Distribución por Sucursal</CardTitle>
                <CardDescription>Solicitudes por sucursal</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Solicitudes por ciudad</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {informe.por_ciudad.map((ciudad, index) => (
                    <div key={index} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <MapPin className="h-4 w-4 text-gray-500" />
                        <span className="font-medium">{ciudad.sucursal_nombre}</span>
                      </div>
                      <div className="text-right">
                        <div className="font-medium">{formatNumber(ciudad.cantidad_registros)}</div>
                        <div className="text-sm text-gray-500">{ciudad.porcentaje}%</div>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Evolución de las solicitudes en los últimos meses</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
